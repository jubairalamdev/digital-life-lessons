...
const LessonDetailsPage = async ({ params }) => {
    ...
    if (!lessonData) {
        return (
            <div className="w-full min-h-screen bg-white dark:bg-zinc-950 flex flex-col items-center justify-center px-4 text-center">
                <p className="text-zinc-400 dark:text-zinc-500 text-sm font-light">The requested life lesson could not be resolved or found.</p>
                <Link href="/allLessons" className="mt-4">
                    <Button size="sm" className="bg-zinc-200 dark:bg-zinc-800 text-zinc-900 dark:text-white border border-zinc-300 dark:border-zinc-700 rounded-xl">
                        Back to Feed
                    </Button>
                </Link>
            </div>
    ...
    return (
        <main className="w-full min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 py-16 relative overflow-hidden">
            ...
            <div className="container mx-auto max-w-4xl px-4 relative z-10 space-y-8">
                ...
                <div>
                    <Link href="/allLessons" className="text-xs text-zinc-400 dark:text-zinc-500 hover:text-green-400 transition-colors inline-flex items-center gap-2">
                        ← Back to all lessons
                    </Link>
                </div>

                {/* CORE SYSTEM CONTAINER BOX */}
                <div className="relative overflow-hidden rounded-3xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-100 dark:bg-zinc-900/10 backdrop-blur-md shadow-2xl">
                    ...
                    <article className={`p-6 md:p-12 space-y-8 ${dynamicLockGate ? 'blur-md select-none pointer-events-none' : ''}`}>
                        ...
                        <div className="flex flex-wrap items-center gap-2.5">
                            <span className="text-[11px] font-bold tracking-wider uppercase px-3 py-1 rounded-md bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300">
                                {lessonData.category}
                            </span>
                            <span className={`text-[11px] font-bold tracking-wider uppercase px-3 py-1 rounded-md border ${toneColors[lessonData.emotionalTone] || 'text-zinc-500 dark:text-zinc-400 border-zinc-200 dark:border-zinc-800'}`}>
                                {lessonData.emotionalTone}
                            </span>
                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded ml-auto ${lessonData.accessLevel === 'Premium' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-zinc-200 dark:bg-zinc-800/40 text-zinc-400 dark:text-zinc-500'}`}>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-zinc-400 dark:text-zinc-500 pt-1">
                                ...
                                <span className="flex items-center gap-1.5">
                                    <Clock size={13} className="text-zinc-400 dark:text-zinc-600" />
                                    {calculatedReadTime} min read time
                                </span>
                                <span className="flex items-center gap-1.5">
                                    <Eye size={13} className="text-zinc-400 dark:text-zinc-600" />
                        ...
                        {lessonData.featuredImage && (
                            <div className="w-full h-64 md:h-96 rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 relative">
                        ...
                        <div className="prose prose-invert max-w-none border-t border-zinc-100 dark:border-zinc-900/80 pt-6">
                            <p className="text-zinc-700 dark:text-zinc-300 text-base md:text-lg font-light leading-relaxed whitespace-pre-line tracking-wide">
                                {lessonData.description}
                            </p>
                        </div>

                        {/* 4. Stats Engagement Overview Tracking Grid Block */}
                        <div className="grid grid-cols-3 gap-4 py-4 border-y border-zinc-100 dark:border-zinc-900 font-mono text-xs text-zinc-500 dark:text-zinc-400 text-center">
                            <div>
                                <span className="block text-lg font-bold text-zinc-800 dark:text-zinc-200">{lessonData.likes.length || 0}</span>
                                <span className="text-zinc-400 dark:text-zinc-600 text-[10px] uppercase tracking-wider">Likes Counter</span>
                            </div>
                            <div>
                                <span className="block text-lg font-bold text-zinc-800 dark:text-zinc-200">{lessonData.favoritesCount || 14}</span>
                                <span className="text-zinc-400 dark:text-zinc-600 text-[10px] uppercase tracking-wider">Bookmarked</span>
                            </div>
                            <div>
                                <span className="block text-lg font-bold text-zinc-800 dark:text-zinc-200">{displayViews}</span>
                                <span className="text-zinc-400 dark:text-zinc-600 text-[10px] uppercase tracking-wider">Traffic Impressions</span>
                            </div>
                        </div>
                ...
                <div className="bg-zinc-100 dark:bg-zinc-900/10 border border-zinc-100 dark:border-zinc-900 rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                            <span className="text-xs text-zinc-400 dark:text-zinc-500 font-medium block">Shared by Thinker:</span>
                            <h4 className="text-base font-bold text-zinc-900 dark:text-white tracking-tight">{userProfile?.name || "Anonymous Member"}</h4>
                            <span className="text-[11px] font-mono text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-900 px-2 py-0.5 rounded border border-zinc-200 dark:border-zinc-800/80">
                                ✍️ {rawLessons.length} Lessons Published
                            </span>
                        </div>
                    </div>
                    <Link href={`/profile/${userProfile?._id || targetId}`}>
                        <Button size="sm" className="bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 text-xs rounded-xl h-9">
                            View All by Author
                        </Button>
                    </Link>
                </div>
                ...
                <div className="space-y-6 pt-4">
                    <div className="flex items-center gap-2 text-zinc-900 dark:text-white font-bold text-lg tracking-tight">
