...
const HomeContributors = async () => {
  ...
  return (
    <section className="w-full py-16 bg-white dark:bg-zinc-950 border-zinc-100 dark:border-zinc-900 relative">
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="bg-zinc-100 dark:bg-zinc-900/40 backdrop-blur-md border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden  shadow-xl">
          <Table aria-label="Top Contributors Table"  className="text-zinc-700 dark:text-zinc-300">
            <Table.ScrollContainer>
              <Table.Content className="min-w-[600px]">
                <Table.Header>
                  <Table.Column isRowHeader className="bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 font-bold">Name</Table.Column>
                  <Table.Column className="bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 font-bold">Email</Table.Column>
                  <Table.Column className="bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 font-bold">System Role</Table.Column>
                  <Table.Column className="bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 font-bold text-right">Lessons Created</Table.Column>
                </Table.Header>
                <Table.Body>
                  {displayContributors.length > 0 ? (
                    displayContributors.map((user) => (
                      <Table.Row key={user.id} className="border-b border-zinc-200 dark:border-zinc-800/50 dark:hover:bg-zinc-900/50 transition-colors">
                        <Table.Cell className="font-semibold text-zinc-900 dark:text-white py-4">{user.name}</Table.Cell>
                        <Table.Cell className="text-zinc-500 dark:text-zinc-400">{user.email}</Table.Cell>
                        <Table.Cell className="capitalize">
                          <span className={`px-2 py-0.5 rounded text-xs ${user?.role === 'admin' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400'}`}>
                            {user?.role}
                          </span>
                        </Table.Cell>
                    ...
                    <Table.Row>
                      <Table.Cell className="text-center text-zinc-400 dark:text-zinc-500 py-6" colSpan={4}>
